...
- The first committed revision (`faa9c956fcbe_baseline_schema.py`) reflects the schema prior to adopting migrations. When upgrading an existing local database for the first time, back it up and run `ALEMBIC_CONFIG=pyproject.toml alembic stamp faa9c956fcbe && ALEMBIC_CONFIG=pyproject.toml alembic upgrade head` to mark and apply the baseline safely.
