...
## Configuration
- The application/composition layer resolves the database URI via `sortipy.config.get_database_config()` (honouring `DATABASE_URI` or falling back to the default file location under `SORTIPY_DATA_DIR`) and passes it into the SQLAlchemy adapter. Adapters do not read environment variables directly.
...
- `env.py` is the environment hook that Alembic executes for each migration command. It ensures SQLAlchemy mappers are configured, resolves a database URL with `get_database_config()` when the CLI command does not supply one, and executes migrations using a temporary engine built from the project settings.
