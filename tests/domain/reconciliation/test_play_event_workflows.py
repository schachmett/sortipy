...
def test_reconcile_play_events_skips_commit_when_empty() -> None:
    repo = FakePlayEventRepository()
...
def test_reconcile_play_events_skips_existing_timestamps() -> None:
    event = make_play_event()
    user = event.user
...
def test_reconcile_play_events_respects_from_timestamp() -> None:
...
def test_reconcile_play_events_returns_now_playing_without_persisting() -> None:
...
def test_reconcile_play_events_respects_to_timestamp_upper_bound() -> None:
...
def test_reconcile_play_events_reports_latest_timestamp_from_new_data() -> None:
    base_time = datetime.now(tz=UTC).replace(microsecond=0)
...
def test_reconcile_play_events_uses_repository_latest_timestamp_when_request_missing_from() -> None:
    base_time = datetime.now(tz=UTC).replace(microsecond=0)
...
def test_reconcile_play_events_honours_max_events_limit() -> None:
    base_time = datetime.now(tz=UTC).replace(microsecond=0)
    events = [
...
def test_reconcile_play_events_skips_commit_when_upper_bound_filters_all_events() -> None:
    base_time = datetime.now(tz=UTC).replace(microsecond=0)
...
def test_reconcile_play_events_deduplicates_events_with_same_timestamp_in_batch() -> None:
    base_time = datetime.now(tz=UTC).replace(microsecond=0)
