...
def test_build_catalog_claim_graph_tracks_entities_associations_and_roots() -> None:
    ...
    graph_result = build_catalog_claim_graph(roots=(recording,), source=Provider.LASTFM)

    assert graph_result.graph.roots == (
        graph_result.graph.require_claim(graph_result.require_entity_claim_id(recording)),
    )
    assert graph_result.graph.claim_for(graph_result.require_entity_claim_id(artist)) is not None
    assert (
        graph_result.graph.claim_for(graph_result.require_entity_claim_id(release_set)) is not None
    )
    assert graph_result.graph.claim_for(graph_result.require_entity_claim_id(release)) is not None
    assert (
        graph_result.graph.relationship_for(graph_result.require_association_claim_id(contribution))
        is not None
    )
    assert (
        graph_result.graph.relationship_for(graph_result.require_association_claim_id(track))
        is not None
    )


def test_catalog_claim_graph_result_resolves_materialized_objects_via_representatives() -> None:
    ...
    graph_result = build_catalog_claim_graph(roots=(original, duplicate), source=Provider.SPOTIFY)
    original_claim_id = graph_result.require_entity_claim_id(original)
    duplicate_claim_id = graph_result.require_entity_claim_id(duplicate)
    ...
    assert (
        graph_result.require_materialized_entity(
            original,
